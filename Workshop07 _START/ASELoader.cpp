...
void ASELoader::readTextureList(vector<Vertex3> &textureCoordinates)
{
	char line[255];
	string currentLine = line;
...
void ASELoader::readTextureFaceList(vector<int> &textureIndices)
{
	char line[255];
	string currentLine = line;
	...
	do
	{
		if (currentLine.find("*MESH_TFACE ")!= string::npos)
		{
			...
			textureIndices.push_back(a);
			textureIndices.push_back(b);
			textureIndices.push_back(c);
		}
		m_modelFile.getline(line,255);
		currentLine = line;
	} while (currentLine.find("}") == string::npos);
