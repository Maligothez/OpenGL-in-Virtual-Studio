...
void RendererOpenGL::Render(Excavator &bigExcavator, vector<Geometry> &things, vector<Geometry> &sky) // this does the drawing
{	
	...
	bool fps = bigExcavator.getFPSMode();
	...
	// set up camera coordinates and point the camera at (0,0,0)
		...
		//cameraHeight = (15*pow(speed,3)) + 100; //+120;
	...
	glLoadIdentity(); // clear coordinate transformation - (0,0,0) is the screen centre
	// clear depth buffer first - so text draws over the top
	glTranslatef(0,2,-10);
...
void RendererOpenGL::initialise()
{
    ...
    glClearColor(1.0,1.0,1.0,0.0);						// select what colour the background is (here set to black)
